...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const titulo = `Nueva solicitud #${numero || "—"}`;
    const cuerpo = `${area} · ${maquina} — ${solicitante}`;

    const supabase = createClient(supabaseUrl, serviceKey);
    const { data: destinatarios, error: errUsers } = await supabase
    ...
    return new Response(JSON.stringify({ sent, tokens: lista.length, stale: staleIds.length }), {
      headers: { "Content-Type": "application/json" },
    });
  } catch (e) {
