/**
 * Edge Function: avisa por FCM al operario cuando el admin le asigna una solicitud.
 *
 * Despliegue:
 *   supabase functions deploy avisar-asignacion-correctivo-push --no-verify-jwt
 *
 * Webhook: supabase/migrations/webhook_avisar_asignacion_correctivo_push.sql
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // El operario se asignó solo (TOMAR): no hace falta push.
    if (record?.origen === "claim") {
      return new Response(JSON.stringify({ skipped: "claim" }), {
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (record?.correctivo_id) {
      const { data: corr } = await supabase
        .from("correctivo")
        .select("datos")
        .eq("id", record.correctivo_id)
        .maybeSingle();
      const datos = (corr?.datos ?? {}) as Record<string, unknown>;
    ...
    const titulo = `Solicitud asignada #${numero}`;
    const cuerpo = `${area} · ${maquina}`;
    ...
    for (const row of lista) {
      const result = await enviarFcm(accessToken, sa.project_id, row.token, titulo, cuerpo, {
        area,
        registroId: record?.correctivo_id || "",
