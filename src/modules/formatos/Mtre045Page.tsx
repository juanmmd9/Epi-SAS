...
import { generarPdfMtre045, urlVistaPreviaPdf } from "./mtre045Pdf";
import {
...
function Mtre045Page() {
  ...
  function cargarDesdePreventivo(registro: RegistroPreventivo) {
    ...
    setDatos(prefillMtre045DesdePreventivo(prefill));
    setMensaje(`Cargado PM del ${registro.fecha} — ${prefill.equipo}`);
  ...
  async function manejarImprimir() {
    ...
    setError(null);
    try {
      const bytes = await generarPdfMtre045(datos);
      mostrarPreview(bytes);
      imprimirPdf(bytes);
      setMensaje("Vista previa generada. Archive el documento firmado en carpeta física.");
    } catch (e) {
  ...
  async function manejarGuardarEnPreventivo() {
    ...
    try {
      ...
      await actualizarPreventivo(datos.preventivoId, {
        datos: {
          ...registro.datos,
          mtre045: datos,
        },
  ...
  function nombreEquipoPm(registro: RegistroPreventivo): string {
    const hoja = hojas.find((h) => h.id === registro.hoja_id);
    return hoja?.nombre ?? registro.datos.equipo ?? "Equipo";
  }
    ...
    <section className="formatos mtre045-page">
      <header className="formatos__cabecera">
        <div>
          ...
          <p className="formatos__descripcion">
            Formato oficial laboratorio. Complete los datos, use{" "}
            <strong>Imprimir</strong> para ver la vista previa y abrir el diálogo de impresión.
          </p>
        </div>
      ...
      <div className="mtre045-page__layout">
        <form className="mtre045-form" onSubmit={manejarEnvioFormulario}>
          ...
          <div className="mtre045-form__grid">
            ...
            <label>
              ...
              <input
                value={datos.serie}
                onChange={(e) => actualizar({ serie: e.target.value })}
              />
            </label>
            <label>
              Área
            ...
            <label className="mtre045-form__ancho">
              Cambio de repuestos o insumos (PM)
              <textarea
                rows={2}
                value={datos.cambioRepuestos}
                onChange={(e) => actualizar({ cambioRepuestos: e.target.value })}
              />
            </label>
            <label className="mtre045-form__ancho">
              ...
              <textarea
                ...
                onChange={(e) => actualizar({ verificacionEquipoPm: e.target.value })}
              />
            </label>
            <label className="mtre045-form__ancho">
              ...
              <textarea
                ...
                onChange={(e) => actualizar({ actividadCorrectivo: e.target.value })}
              />
            </label>
            <label className="mtre045-form__ancho">
              Cambio repuestos (correctivo)
              <textarea
                rows={2}
                value={datos.cambioRepuestosCorrectivo}
                onChange={(e) => actualizar({ cambioRepuestosCorrectivo: e.target.value })}
              />
            </label>
            <label>
              Inspección visual
              <select
                ...
                onChange={(e) =>
                  actualizar({ inspeccionVisual: e.target.value as VerificacionAn })
                }
              >
                <option value="">—</option>
            ...
            <label>
              ...
              <select
                ...
                onChange={(e) =>
                  actualizar({ pruebasFuncionamiento: e.target.value as VerificacionAn })
                }
              >
                <option value="">—</option>
                <option value="A">A — Aprobado</option>
            ...
            <label className="mtre045-form__ancho">
              ...
              <input
                ...
                onChange={(e) => actualizar({ noAprobo: e.target.value })}
              />
            </label>
            <label>
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) => actualizar({ responsableVerificacion: e.target.value })}
              />
            </label>
          </div>
        ...
        <aside className="mtre045-lista-pm">
          ...
          <p className="formatos__descripcion">
            Filtra por área y fecha, luego carga el PM para llenar el reporte.
          </p>
            ...
            {registrosPmFiltrados.map((registro) => (
              <article key={registro.id} className="mtre045-item-pm">
                <div>
                  ...
                  <p>
                    <em>{nombreEquipoPm(registro)}</em>
                    {" · "}
                    {registro.descripcion?.slice(0, 60) ?? "Sin descripción"}
                  </p>
                </div>
                <button type="button" className="btn" onClick={() => cargarDesdePreventivo(registro)}>
                  Cargar
                </button>
              </article>
            ))}
      ...
      {pdfPreview && (
        <section ref={previewRef} className="permisos__preview">
          ...
          <iframe title="Vista previa MT-RE-045" src={pdfPreview} className="permisos__iframe" />
        </section>
      )}
    </section>
  );
